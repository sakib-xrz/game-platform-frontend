...
export function ConfigEditor({ initial, submitLabel, onSave, readOnly = false, validateConfig = adminClient.validateConfig, uploadAsset = adminClient.uploadAsset, gameLabel = "Greedy", lockOptionCodes = false }: { initial: CreateAdminConfigInput; submitLabel: string; onSave: (payload: CreateAdminConfigInput) => Promise<void>; readOnly?: boolean; validateConfig?: (payload: CreateAdminConfigInput) => Promise<ConfigValidationPreview>; uploadAsset?: (file: File) => Promise<import("@/types/admin").AdminAsset>; gameLabel?: string; lockOptionCodes?: boolean }) {
  ...
  return <form className="space-y-6" onSubmit={readOnly ? (event) => event.preventDefault() : handleSubmit(submit)}>
    ...
    <Card><CardHeader><CardTitle>{gameLabel} options</CardTitle><CardDescription>Manage every outcome, probability weight, payout multiplier, display order and artwork.</CardDescription></CardHeader><CardContent className="grid gap-4 xl:grid-cols-2">
