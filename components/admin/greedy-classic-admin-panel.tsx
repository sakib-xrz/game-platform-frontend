...
export function GreedyClassicAdminPanel() {
  ...
  const runtime = useQuery({
    queryKey: ["admin", "greedy-classic", "runtime"],
    queryFn: adminClient.greedyClassic.runtime,
    refetchInterval: 5000,
  });
  const configs = useQuery({
    queryKey: ["admin", "greedy-classic", "configs"],
    queryFn: adminClient.greedyClassic.configs,
  });
  const approvals = useQuery({
    queryKey: ["admin", "approvals", "greedy-classic"],
    queryFn: () => adminClient.approvalsPaged("?page=1&limit=100"),
    refetchInterval: 8000,
  });
  const canManage =
    identity.role === "super_admin" || identity.role === "game_operator";
  const greedyClassicApprovals = useMemo(
    () =>
      (approvals.data?.data || []).filter(
        (item) => item.action_type === "greedy_classic.config.publish",
      ),
    [approvals.data],
  );
  ...
  async function refresh() {
    await Promise.all([
      queryClient.invalidateQueries({ queryKey: ["admin", "greedy-classic"] }),
  ...
  const action = useMutation({
    mutationFn: async (next: "resume" | "pause") =>
      next === "resume"
        ? adminClient.greedyClassic.resume()
        : adminClient.greedyClassic.pause(),
    onSuccess: async (_data, next) => {
      toast.success(
        next === "resume"
          ? "Greedy Classic resumed"
          : "Greedy Classic paused safely",
      );
      await refresh();
    },
    onError: (reason) =>
  ...
  const publish = useMutation({
    mutationFn: (id: string) => adminClient.greedyClassic.publish(id),
  ...
  const decide = useMutation({
    mutationFn: ({
      ...
      decision === "approve"
        ? adminClient.approve(approval.id, "Reviewed from Manage Greedy Classic")
        : adminClient.reject(approval.id, "Rejected from Manage Greedy Classic"),
  ...
  const applyApproved = useMutation({
    mutationFn: adminClient.greedyClassic.publishApproved,
    onSuccess: async () => {
      toast.success("Approved Greedy Classic configuration is now live");
      await refresh();
    },
    onError: (reason) =>
      toast.error(
  ...
  async function save(payload: CreateAdminConfigInput) {
    ...
    if (editor.mode === "create")
      await adminClient.greedyClassic.createConfig(payload);
    else await adminClient.greedyClassic.updateConfig(editor.config.id, payload);
    toast.success(
      editor.mode === "create"
  ...
  const editorTitle =
    editor?.mode === "create"
      ? "Create Greedy Classic configuration"
  ...
  if (runtime.isError || configs.isError)
    return (
      <Alert variant="destructive">
        <AlertTitle>Greedy Classic administration is unavailable</AlertTitle>
        <AlertDescription>
    ...
    <div className="space-y-6">
      ...
      {!canManage && (
        <Alert variant="warning">
          ...
          <AlertDescription>
            Your admin role can inspect Greedy Classic configuration but cannot
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>
              Create, manage, clone and publish complete Greedy Classic
              configurations.
            </CardDescription>
          </div>
          {/* <Button
            disabled={!canManage}
            onClick={() =>
              setEditor({ mode: "create", initial: newConfigDefaults })
            }
          >
      ...
      <Card>
        <CardHeader>
          <CardTitle>Greedy Classic publish approvals</CardTitle>
          <CardDescription>
        ...
        <CardContent>
          {greedyClassicApprovals.length ? (
            <Table>
              ...
              <TableBody>
                {greedyClassicApprovals.map((approval) => {
                  const requestedByMe =
            ...
            <p className="text-sm text-slate-500">
              No Greedy Classic publish approval requests are visible to this
              account.
            </p>
          )}
        </CardContent>
          ...
          {editor && (
            <div className="space-y-4 px-4 pb-6">
              <ConfigEditor
                ...
                readOnly={!canManage}
                onSave={save}
                validateConfig={adminClient.greedyClassic.validateConfig}
                uploadAsset={adminClient.greedyClassic.uploadAsset}
              />
            </div>
          )}
        </SheetContent>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              approved and applied, the Greedy Classic user panel updates from
