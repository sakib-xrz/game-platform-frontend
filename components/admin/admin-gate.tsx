...
export function AdminGate({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { if (pathname === "/admin/login") return; let active = true; fetch("/api/admin/session", { credentials: "include", cache: "no-store" }).then(async (response) => { if (!response.ok) { router.replace("/admin/login"); return; } const body = await response.json() as { data: AdminSession }; if (active && body.data.identity.force_password_change && pathname !== "/admin/password") { router.replace("/admin/password"); return; } if (active) setState(body.data); }).catch(() => router.replace("/admin/login")).finally(() => { if (active) setLoading(false); }); return () => { active = false; }; }, [pathname, router]);
  if (pathname === "/admin/login") return <>{children}</>;
  if (loading || !state) return <div className="grid min-h-dvh place-items-center bg-slate-50 text-sm font-medium text-slate-600">Verifying secure session…</div>;
