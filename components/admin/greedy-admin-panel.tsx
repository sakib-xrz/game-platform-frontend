...
export function GreedyAdminPanel() {
  ...
  const runtime = useQuery({
    queryKey: ["admin", "greedy", "runtime"],
    queryFn: adminClient.runtime,
    refetchInterval: 5000,
  });
  const configs = useQuery({
    queryKey: ["admin", "greedy", "configs"],
    queryFn: adminClient.configs,
  });
  const approvals = useQuery({
    queryKey: ["admin", "approvals", "greedy"],
    queryFn: () => adminClient.approvalsPaged("?page=1&limit=100"),
    refetchInterval: 8000,
  });
  const canManage =
    identity.role === "super_admin" || identity.role === "game_operator";
  const greedyApprovals = useMemo(
    () =>
      (approvals.data?.data || []).filter(
        (item) => item.action_type === "greedy.config.publish",
      ),
    [approvals.data],
  );
  ...
  async function refresh() {
    await Promise.all([
      queryClient.invalidateQueries({ queryKey: ["admin", "greedy"] }),
      queryClient.invalidateQueries({ queryKey: ["admin", "approvals"] }),
    ]);
  }
  const action = useMutation({
    mutationFn: async (next: "resume" | "pause") =>
      next === "resume" ? adminClient.resume() : adminClient.pause(),
    onSuccess: async (_data, next) => {
      toast.success(
        next === "resume" ? "Greedy resumed" : "Greedy paused safely",
      );
      await refresh();
    },
    onError: (reason) =>
  ...
  const publish = useMutation({
    mutationFn: (id: string) => adminClient.publish(id),
  ...
  const decide = useMutation({
    mutationFn: ({
      ...
      decision === "approve"
        ? adminClient.approve(approval.id, "Reviewed from Manage Greedy")
        : adminClient.reject(approval.id, "Rejected from Manage Greedy"),
  ...
  const applyApproved = useMutation({
    mutationFn: adminClient.publishApproved,
    onSuccess: async () => {
      toast.success("Approved Greedy configuration is now live");
      await refresh();
    },
    onError: (reason) =>
      toast.error(
  ...
  async function save(payload: CreateAdminConfigInput) {
    if (!editor) return;
    if (editor.mode === "create") await adminClient.createConfig(payload);
    else await adminClient.updateConfig(editor.config.id, payload);
    toast.success(
      editor.mode === "create"
  ...
  if (runtime.isError || configs.isError)
    return (
      <Alert variant="destructive">
        <AlertTitle>Greedy administration is unavailable</AlertTitle>
        <AlertDescription>
    ...
    <div className="space-y-6">
      ...
      {!canManage && (
        <Alert variant="warning">
          ...
          <AlertDescription>
            Your admin role can inspect Greedy configuration but cannot change
            runtime or settings.
          </AlertDescription>
        </Alert>
      )}
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>
              Create, manage, clone and publish complete Greedy configurations.
            </CardDescription>
          </div>
          {/* <Button
            disabled={!canManage}
      ...
      <Card>
        <CardHeader>
          <CardTitle>Greedy publish approvals</CardTitle>
          <CardDescription>
        ...
        <CardContent>
          {greedyApprovals.length ? (
            ...
            <p className="text-sm text-slate-500">
              No Greedy publish approval requests are visible to this account.
            </p>
          )}
        </CardContent>
      </Card>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              approved and applied, the Greedy user panel updates from its
