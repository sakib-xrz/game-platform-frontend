...
import { TeenPattiConfigEditor } from "@/components/admin/teen-patti-config-editor";
...
import type {
  ...
  TeenPattiAdminConfigVersion,
} from "@/types/admin";

type EditorState =
  | { mode: "create"; initial: CreateTeenPattiAdminConfigInput }
  | { mode: "edit"; config: TeenPattiAdminConfigVersion };

function configInput(
  config: TeenPattiAdminConfigVersion,
): CreateTeenPattiAdminConfigInput {
  return {
    ...
    options: config.options.map(
      ({ code, name, image_url, asset_id, display_order, is_enabled }) => ({
        code,
        name,
        image_url,
        asset_id,
...
function openEditorForConfig(config: TeenPattiAdminConfigVersion): EditorState {
  return { mode: "edit", config };
}

function saveLabel(config?: TeenPattiAdminConfigVersion) {
  if (!config) return "Create draft";
  return config.status === "published" ? "Save changes" : "Save draft";
}

export function TeenPattiAdminPanel() {
  const identity = useAdminIdentity();
  const queryClient = useQueryClient();
  const [editor, setEditor] = useState<EditorState | null>(null);
  const [publishTarget, setPublishTarget] =
    useState<TeenPattiAdminConfigVersion | null>(null);

  const runtime = useQuery({
    queryKey: ["admin", "teen-patti", "runtime"],
    queryFn: adminClient.teenPatti.runtime,
    refetchInterval: 5000,
  });
  const configs = useQuery({
    queryKey: ["admin", "teen-patti", "configs"],
    queryFn: adminClient.teenPatti.configs,
  });
  const approvals = useQuery({
    queryKey: ["admin", "approvals", "teen-patti"],
  ...
  const teenPattiApprovals = useMemo(
    () =>
      (approvals.data?.data || []).filter(
        (item) => item.action_type === "teen_patti.config.publish",
      ),
    [approvals.data],
  );
  ...
  async function refresh() {
    await Promise.all([
      queryClient.invalidateQueries({ queryKey: ["admin", "teen-patti"] }),
  ...
  const action = useMutation({
    mutationFn: async (next: "resume" | "pause") =>
      next === "resume"
        ? adminClient.teenPatti.resume()
        : adminClient.teenPatti.pause(),
    onSuccess: async (_data, next) => {
      toast.success(
        next === "resume" ? "Teen Patti resumed" : "Teen Patti paused safely",
      );
      await refresh();
    },
    onError: (reason) =>
  ...
  const publish = useMutation({
    mutationFn: (id: string) => adminClient.teenPatti.publish(id),
  ...
  const decide = useMutation({
    mutationFn: ({
      ...
      decision === "approve"
        ? adminClient.approve(approval.id, "Reviewed from Manage Teen Patti")
        : adminClient.reject(approval.id, "Rejected from Manage Teen Patti"),
  ...
  const applyApproved = useMutation({
    mutationFn: adminClient.teenPatti.publishApproved,
    onSuccess: async () => {
      toast.success("Approved Teen Patti configuration is now live");
      await refresh();
    },
    onError: (reason) =>
      toast.error(
  ...
  async function save(payload: CreateTeenPattiAdminConfigInput) {
    if (!editor) return;
    if (editor.mode === "create")
      await adminClient.teenPatti.createConfig(payload);
    else await adminClient.teenPatti.updateConfig(editor.config.id, payload);
    toast.success(
      editor.mode === "create"
  ...
  const editorTitle =
    editor?.mode === "create"
      ? "Create Teen Patti configuration"
      : `Manage version ${editingConfig?.version}`;
  const editorDescription =
    editor?.mode === "create"
      ? "Configure timing, bet limits, rake, decks, and chips. Save as a draft, then submit for approval."
      : editingConfig?.status === "published"
        ? "Edit timing, bet limits, rake, decks, and chips. Changes apply to this live version immediately."
        : "Edit timing, bet limits, rake, decks, and chips. Save as a draft, then submit for approval.";
  ...
  if (runtime.isError || configs.isError)
    return (
      <Alert variant="destructive">
        <AlertTitle>Teen Patti administration is unavailable</AlertTitle>
        <AlertDescription>
    ...
    <div className="space-y-6">
      ...
      {!canManage && (
        <Alert variant="warning">
          ...
          <AlertDescription>
            Your admin role can inspect Teen Patti configuration but cannot
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>
              Create, manage, clone, and publish complete Teen Patti
              configurations.
            </CardDescription>
          </div>
          {/* <Button
            disabled={!canManage}
            onClick={() =>
              setEditor({ mode: "create", initial: newTeenPattiConfigDefaults })
            }
          >
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>Decks / chips</TableHead>
                ...
                <TableRow key={config.id}>
                  ...
                  <TableCell>
                    <Button
                      ...
                      onClick={() => setEditor(openEditorForConfig(config))}
                    >
                      {formatRakePercent(config.rake_bps)}
                    </Button>
                  </TableCell>
                  <TableCell>
                    <Button
      ...
      <Card>
        <CardHeader>
          <CardTitle>Teen Patti publish approvals</CardTitle>
          <CardDescription>
        ...
        <CardContent>
          {teenPattiApprovals.length ? (
            <Table>
              ...
              <TableBody>
                {teenPattiApprovals.map((approval) => {
                  const requestedByMe =
            ...
            <p className="text-sm text-slate-500">
              No Teen Patti publish approval requests are visible to this
              account.
            </p>
          )}
        </CardContent>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              approved and applied, the Teen Patti player panel updates from its
