...
export async function POST(request: Request, { params }: { params: Promise<{ alertId: string; action: string }> }) {
  ...
  return proxyAdminRequest(`/admin/games/greedy-classic/alerts/${encodeURIComponent(alertId)}/${action}`, session, { method: "POST", headers: forwardMutationHeaders(request) });
}
