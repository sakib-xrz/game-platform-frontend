...
export async function POST(request: Request, { params }: { params: Promise<{ configId: string }> }) {
  ...
  return proxyAdminRequest(`/admin/games/greedy/config-versions/${encodeURIComponent(configId)}/publish`, session, { method: "POST", headers: forwardMutationHeaders(request) });
}
